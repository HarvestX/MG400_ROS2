...
#include "mg400_plugin/dashboard_api/reset_robot.hpp"
  ...
  const mg400_interface::MG400Interface::SharedPtr mg400_if)
{
  ...
  this->srv_ = rclcpp::create_service<ServiceT, CallbackT>(
    ...
    "reset_robot",
    std::bind(&ResetRobot::onServiceCall, this, _1, _2),
    rclcpp::ServicesQoS().get_rmw_qos_profile(),
    this->node_base_if_->get_default_callback_group());
}

void ResetRobot::onServiceCall(
  ...
  if (this->mg400_interface_->ok()) {
    try {
      plugin_utils::exitServoModeBeforeDashboardStopCommand(
        this->mg400_interface_, this->node_logging_if_->get_logger(), "ResetRobot");
