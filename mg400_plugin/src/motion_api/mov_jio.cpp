...
#include "mg400_plugin/motion_api/mov_jio.hpp"

namespace mg400_plugin
{

void MovJIO::configure(
  ...
  const mg400_interface::MG400Interface::SharedPtr mg400_if)
{
  ...
  this->action_server_ =
    ...
    "mov_jio",
    std::bind(&MovJIO::handle_goal, this, _1, _2),
    std::bind(&MovJIO::handle_cancel, this, _1),
    std::bind(&MovJIO::handle_accepted, this, _1),
    rcl_action_server_get_default_options(),
    this->node_base_if_->get_default_callback_group());
}

rclcpp_action::GoalResponse MovJIO::handle_goal(
  const rclcpp_action::GoalUUID &, ActionT::Goal::ConstSharedPtr goal)
{
  ...
  if (plugin_utils::rejectMotionCommandDuringServoMode(
      this->mg400_interface_, this->node_logging_if_->get_logger(), "MovJIO"))
  {
    return rclcpp_action::GoalResponse::REJECT;
  }
...
rclcpp_action::CancelResponse MovJIO::handle_cancel(
  const std::shared_ptr<GoalHandle>)
{
  RCLCPP_INFO(
    this->node_logging_if_->get_logger(), "Received request to cancel goal");
  // TODO(anyone): Should stop movJIO
  return rclcpp_action::CancelResponse::ACCEPT;
}

void MovJIO::handle_accepted(
  const std::shared_ptr<GoalHandle> goal_handle)
{
  using namespace std::placeholders;  // NOLINT
  std::thread{std::bind(&MovJIO::execute, this, _1), goal_handle}.detach();
}


void MovJIO::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
