  ...
  const mg400_interface::RealtimeFeedbackTcpInterface::SharedPtr rt_if)
{
  ...
  handler = std::make_shared<mg400_interface::PoseHandler>(
    node->get_node_clock_interface(), node->get_node_logging_interface());
  handler->configure();
  handler->setDistFrameId(this->realtime_tcp_interface_->frame_id_prefix + "mg400_origin_link");
  handler->activate();
...
void MovJ::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
  ...
  in = goal->pose;
  this->handler->tfHeader2Dist(in, out);
